...
if ($All) {
    ...
    $unindexed = Get-ChildItem -Path $seedsDir -Filter "*.md" | Where-Object { $_.Name -ne "index.md" -and $_.Name -ne "README.md" } | Where-Object {
...
$latestTemplate = Get-ChildItem -Path $templateDir -Directory | Where-Object { $_.Name -match '^v\d+$' } | Select-Object @{Name="Ver"; Expression={[int]($_.Name -replace 'v','')}} | Sort-Object Ver -Descending | Select-Object -First 1

if (-not $latestTemplate) { 
    Write-Error "No template found in tools/gardener/template!"
    exit 
}

$templateVersion = $latestTemplate.Ver
$templateContent = Get-Content "$templateDir\v$templateVersion\entity.md" -Raw
